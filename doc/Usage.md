...
`fbn` est destiné à être utilisé en ligne de commande (Windows ou Linux) et génère les affichages sur les sorties standard. 
Le fichier CSV est automatiquement généré au terme de l'exécution.
...
Les données sont au format CSV, séparé par le caractère `,` et ou les chaines de caractères sont encadrées par des guillemets: `"`.
...
Le fichier est détections liste le détail de chaque entrée détectée par FastFind.
...
* `FileCreation`       	        date de creation de fichier
...
* `AltFilenameCreation`	        date de cration du nom alternatif de fichier
...
Le fichier est au format MACB couremment utilisé par les outils d'enquête forensique.
...
* `Extension` extension du fichier (derniers caractères après le `.`)
...
* `FileNameCreationDate` date de creation de l'entrée `$FN`
...
* `FileNameLastAttrModificationDate` date de derniere modification des attributs de l'entrée `$FN`
* `MD5`  condensat MD5 encodé en hexadécimal
* `SHA1` condensat SHA1 encodé en hexadécimal
* `SHA256` condensat SHA256 encodé en hexadécimal
...
Le format MACB est une chaine de 4 caractère qui définit les changements constatés sur les méta-informations de l'entrée.
...
* condensat
* taille de fichier
* jour de creation et dernière modification
* mois de creation, et dernière modification 
